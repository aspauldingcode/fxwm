...
#define OVERLAY_CONFIG_PATH "/var/protein/overlays/overlay_list.conf"
...
int create_or_remount_overlay_symlinks(const char *path, char *out_store_path, size_t out_size);
...
int reapply_all_overlays(void) {
    ...
    if (ret != 0 || list == NULL || count == 0) {
        fprintf(stderr, "No overlays to reapply.\n");
        return -1;
    }
    
    int success = 0;
    for (int i = 0; i < count; i++) {
        char backing_path[PATH_MAX] = {0};
        fprintf(stdout, "Reapplying overlay on %s...\n", list[i].target_path);
        
        kern_return_t r = create_or_remount_overlay_symlinks(list[i].target_path, backing_path, sizeof(backing_path));
        if (r == 0) {
            fprintf(stdout, "Successfully reapplied overlay: %s → %s\n", list[i].target_path, backing_path);
            success++;
        } else {
            fprintf(stderr, "Failed to reapply overlay on %s\n", list[i].target_path);
        }
    }
...
int create_or_remount_overlay_symlinks(const char *path, char *out_store_path, size_t out_size) {
    ...
    // Remove the early return for existing tmpfs mounts - always recreate
    
    if (is_mount_point(path) && !is_tmpfs_mount(path)) {
        fprintf(stderr, "Path %s is already a mount point (non-tmpfs). Cannot override.\n", path);
        return -1;
    }
    ...
    // If overlay already exists, unmount and delete the backing store
    if (is_tmpfs_mount(path)) {
        fprintf(stdout, "Removing existing overlay on %s...\n", path);
        unmount_if_mounted(path);
        
        // Delete the backing store
        if (remove_directory_recursive(store_path) != 0) {
            fprintf(stderr, "Warning: Failed to remove old backing store at %s\n", store_path);
        }
    }
    ...
    // Unmount if mounted (handles any remaining mounts)
    unmount_if_mounted(path);
    ...
    write_overlay_config(&ov);
    
    fprintf(stdout, "Overlay (symlink-based) created on '%s' with backing '%s'\n", path, store_path);
    
    if (out_store_path && out_size > 0)
        strlcpy(out_store_path, store_path, out_size);
...
static kern_return_t copy_file_contents(const char *src, const char *dst) {
...
static kern_return_t copy_dir_recursive(const char *src, const char *dst) {
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        if (S_ISDIR(st.st_mode) && !S_ISLNK(st.st_mode)) {
            ...
            ret = copy_file_contents(src_path, dst_path);
            if (ret != 0) {
                closedir(dir);
                return ret;
            }
...
static kern_return_t write_overlay_config(const overlay_info_t *overlay) {
    ...
    kern_return_t ret = ensure_directory_exists("/var/protein/overlays");
    if (ret != 0) {
        return ret;
    }
